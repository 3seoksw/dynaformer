...
class CurrentProfile:
        ...
        if type == "constant_currents":
            ...
            if current_profiles is None:
                self.current_profiles = [
                    self.get_scalar_current(self.min_val, self.max_val)
            ...
            raise ValueError(
                "Invalid type, must be either variable_currents or constant_currents"
            )
            # Gene
    ...
    def generate(self, t, x=None, unit_of_measure=1):
        ...
        if self.times_profile == []:
            return {
                "i": self.current_profiles[0]
            }  # We are in the constant loading case
...
def compute_scalers(data_dir, is_val):
    ...
    try:
        ...
        i_c = np.array(
            [
                np.array(
                    x[0].return_current_profile_array(
                        time_denominator=1000, pad_profiles=True
                    )
                )
                for x in i_c
            ]
